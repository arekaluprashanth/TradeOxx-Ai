...
import { Search, Bell, ChevronDown, LogOut, Settings, User, Sun, Moon, TrendingUp, TrendingDown, RefreshCw, BarChart2, Star, } from 'lucide-react';
...
export default function TopNav({ onMenuClick }) {
  ...
  const unreadCount = _optionalChain([notifications, 'optionalAccess', _ => _.filter, 'call', _2 => _2((n) => !n.read), 'access', _3 => _3.length]) || 0;

  const initials = _optionalChain([user, 'optionalAccess', _4 => _4.name])
    ? user.name
        .split(' ')
        .map((n) => n[0])
        .join('')
                ...
                <>
                  ...
                  >
                    ...
                    <div className="max-h-64 overflow-y-auto">
                      {_optionalChain([notifications, 'optionalAccess', _5 => _5.length]) > 0 ? (
                ...
                <>
                  ...
                  >
                    <div className="px-3 py-3 border-b border-white/5 mb-2">
                      <p className="text-sm font-bold text-white">{_optionalChain([user, 'optionalAccess', _6 => _6.name]) || 'Trader'}</p>
                      <p className="text-xs text-dark-400 mt-0.5">{_optionalChain([user, 'optionalAccess', _7 => _7.email]) || 'trader@tradeoxx.ai'}</p>
                    </div>
                    <button 
                ...
                <div className="flex items-center gap-6">
                  <div>
                    ...
                    <span className="font-mono text-sm font-semibold text-white">
                      {formatCurrency(_nullishCoalesce(_optionalChain([portfolio, 'optionalAccess', _8 => _8.totalValue]), () => ( 0)))}
                    </span>
                  </div>
                  <div>
                    <span className="text-dark-400 block mb-0.5 uppercase tracking-wider">Unrealized PnL</span>
                    <span className={`font-mono text-sm font-semibold ${(_nullishCoalesce(_optionalChain([portfolio, 'optionalAccess', _9 => _9.totalPnl]), () => ( 0))) >= 0 ? 'text-accent-green' : 'text-accent-red'}`}>
                      {(_nullishCoalesce(_optionalChain([portfolio, 'optionalAccess', _10 => _10.totalPnl]), () => ( 0))) >= 0 ? '+' : ''}{formatCurrency(_nullishCoalesce(_optionalChain([portfolio, 'optionalAccess', _11 => _11.totalPnl]), () => ( 0)))}
                    </span>
                  </div>
                  <div>
                    ...
                    <span className="font-mono text-sm font-semibold text-accent-cyan">
                      {formatCurrency(_nullishCoalesce(_optionalChain([portfolio, 'optionalAccess', _12 => _12.balance]), () => ( 100000)))}
                    </span>
                  </div>
                </div>
