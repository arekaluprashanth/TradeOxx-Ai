<#
.SYNOPSIS
  Build and export TradeOxx Ai for deployment.
  Produces:
    1. Docker image tar files  -> export/docker/
    2. Static dist ZIP         -> export/TradeOxx Ai-dist.zip
.NOTES
...
Write-Host "   TradeOxx Ai - Build and Export"           -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
...
if (-not $DockerOnly) {
    ...
    $ZipPath = Join-Path $ExportDir "TradeOxx Ai-dist.zip"
    if (Test-Path $ZipPath) { Remove-Item $ZipPath -Force }

    Write-Step "Creating ZIP archive..."
    Compress-Archive -Path "$DistSource\*" -DestinationPath $ZipPath -Force
    $zipSize = [math]::Round((Get-Item $ZipPath).Length / 1MB, 2)
    Write-Done "Dist ZIP created: export\TradeOxx Ai-dist.zip ($zipSize MB)"
...
if (-not $DistOnly) {
    ...
    # Build server image
    Write-Step "Building server image (TradeOxx Ai-server)..."
    cmd /c "cd /d `"$ProjectRoot`" && docker build -t TradeOxx Ai-server:latest -f docker/Dockerfile.server ." 2>&1
    if ($LASTEXITCODE -ne 0) { Write-Err "Server image build failed!"; exit 1 }
    Write-Done "Server image built"

    # Build client image
    Write-Step "Building client image (TradeOxx Ai-client)..."
    cmd /c "cd /d `"$ProjectRoot`" && docker build -t TradeOxx Ai-client:latest -f docker/Dockerfile.client ." 2>&1
    ...
    $ServerTar = Join-Path $DockerExportDir "TradeOxx Ai-server.tar"
    cmd /c "docker save -o `"$ServerTar`" TradeOxx Ai-server:latest" 2>&1
    $serverSize = [math]::Round((Get-Item $ServerTar).Length / 1MB, 2)
    Write-Done "Server tar: export\docker\TradeOxx Ai-server.tar ($serverSize MB)"

    Write-Step "Exporting client image to tar..."
    $ClientTar = Join-Path $DockerExportDir "TradeOxx Ai-client.tar"
    cmd /c "docker save -o `"$ClientTar`" TradeOxx Ai-client:latest" 2>&1
    $clientSize = [math]::Round((Get-Item $ClientTar).Length / 1MB, 2)
    Write-Done "Client tar: export\docker\TradeOxx Ai-client.tar ($clientSize MB)"
    ...
    Write-Host "     2. docker load -i TradeOxx Ai-server.tar" -ForegroundColor DarkGray
    Write-Host "     3. docker load -i TradeOxx Ai-client.tar" -ForegroundColor DarkGray
...
if (-not $DockerOnly) {
    Write-Host "    [ZIP] TradeOxx Ai-dist.zip   - Netlify/Vercel upload" -ForegroundColor White
}
if (-not $DistOnly) {
