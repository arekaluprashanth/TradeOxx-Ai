...
export default function ChartToolbar({
  ...
  const filteredAssets = (assets || []).filter(
    (a) =>
      _optionalChain([a, 'access', _ => _.symbol, 'optionalAccess', _2 => _2.toLowerCase, 'call', _3 => _3(), 'access', _4 => _4.includes, 'call', _5 => _5(assetSearch.toLowerCase())]) ||
      _optionalChain([a, 'access', _6 => _6.name, 'optionalAccess', _7 => _7.toLowerCase, 'call', _8 => _8(), 'access', _9 => _9.includes, 'call', _10 => _10(assetSearch.toLowerCase())])
  );
    ...
    <div className="flex flex-wrap items-center gap-2 bg-dark-800/50 border border-white/5 rounded-xl p-2">
      ...
      <div className="flex items-center gap-1">
        {timeframes.map((tf) => (
          <button
            key={tf}
            onClick={() => _optionalChain([onTimeframeChange, 'optionalCall', _11 => _11(tf)])}
            ...
            >
              ...
              <div className="py-1">
                {indicators.map((ind) => (
                  <button
                    key={ind.id}
                    onClick={() => _optionalChain([onIndicatorToggle, 'optionalCall', _12 => _12(ind.id)])}
                    className="w-full flex items-center justify-between px-3 py-2 text-sm  transition-colors"
                  >
                    <div>
